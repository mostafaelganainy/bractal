...
class RelayForm extends Component {
  static getDerivedStateFromProps(props, state) {
    if (props.options !== state.options) {
      return {
        options: getTcombOptionsFromRawOptions(props.options),
        type: getTcombTypesFromRawOptions(props.options),
      };
    }
  ...
  updatetCompOptionsWithErrors(errors) {
    ...
    options.fields.forEach((option) => {
      if (errors[option.name]) {
        fields[option.name] = {};
        fields[option.name].hasError = {
          $set: true,
        };
        fields[option.name].error = {
          $set: `${errors[option.name]}`,
        };
      }
    });
    this.setState({
      options: t.update(this.state.options.fields, fields),
    });
  }
